...
@dataclass
class ExtCommandResult(CommandResult):
    extra_info: str
...
{"level": "info", "message": "Completed successfully"}'''
    ...
    def given_common_json_cli(self):
        self.json_cli = CommonJsonCli(
            ...
            result_factory=ExtCommandResult,
        )
    ...
    async def when_user_executes_command(self):
        self.result = await self.json_cli.exec(
            ...
            command_result_extra={'extra_info': 'additional data'}
        )
    ...
    def then_result_should_be_command_result(self):
        assert isinstance(self.result, ExtCommandResult)
    ...
    def then_result_should_contain_extra_info(self):
        assert self.result.extra_info == 'additional data'
