...
"""
    ...
    async def given_external_definition(self):
        self.env_vars = {'ENV_VAR_HOST': 'external_s2_%(test_run_id)s_end'}
    ...
    async def and_it_should_have_env_var_set(self):
        ...
        self.test_run_id_interpolation = {'test_run_id': self.uber_compose.run_id}
        self.expected_envs = [f'{k}={v % self.test_run_id_interpolation}' for k, v in self.env_vars.items()]
    ...
    async def then_it_should_apply_migration(self):
        ...
        assert self.migration_file_content == schema.bytes % (
            b'envvar-external_s2_' + str(self.uber_compose.run_id).encode() + b'_end\n'
        )
